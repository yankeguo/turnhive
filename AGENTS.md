...
- 用户文件输入（`agent/uploads.go` + `POST /v1/sessions/{id}/files`）：与 persist 对称的反向通道。turnhive 与调用方**共享同一个 S3 bucket，文件一律以 object_key 传递，turnhive 不中转字节**——调用方自行 PUT 到 bucket，再在任意时刻 `POST files` 登记 `{name, object_key, size}`（name 校验 `^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}$`，session 内按 name 去重，上限 64 个）。记录为 session 字段 `uploads` 并落盘 `sessions/{id}/files.json`（best-effort，DELETE 不删，adopt 时加载恢复），随 `sync` 帧 `files` 字段下发。注入：有活沙箱立即 `InjectUploads`（`PresignGet` + `/agent/v1/file?url=` → `./.agents/uploads/<name>`，30s 上限，失败 502 不登记），冷 session 等 `ensureSandbox` 时全量注入（与 RestorePersisted 并列）。**消息中是否/如何引用文件完全由上游决定**：turnhive 只约定文件固定位于 `./.agents/uploads/<name>`，不规定 marker 格式，`POST messages` 的 content 原样入历史。审计/统计用 `GET /v1/sessions/{id}` 返回 `{id, turn_id, files, persisted}`（无需开 SSE）。
...
- 出口调用统一携带固定头 `X-Turnhive-Session: <session-id>`（`agent.SessionHeader`，注入点在 `agent/loop.go`：LLM 每个 step 请求与 MCP 每次连接都经 `withSessionHeader` 注入，**spec 里的同名头大小写不敏感地被丢弃**，防冒名其他 session）。上游可据此反推用户/权限做门控与归因（内网信任），或忽略并让 turnhive 直连 provider/MCP；门控决策权完全在上游部署侧，turnhive 不做回调。
