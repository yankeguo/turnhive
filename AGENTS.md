...
- 跨节点路由靠 etcd 归属记录 + `X-Turnhive-Forwarded` 头防转发循环；已转发的请求绝不二次转发。节点 keepalive 中断（租约失效）后 registry 会自动退避重试重新注册节点记录（1s 起步、上限 30s，直到成功或 Close）；已丢失的 session 归属记录不恢复。
- LLM 历史只持久化 {user, assistant} 对（S3 JSONL），tool 交互是 turn 内瞬态。失败 turn（流式错误、max steps 超限）同样持久化 {user, assistant-partial}，`failTurn` 用 `context.WithoutCancel` + 10s 超时脱离已取消的 turn ctx 保存。历史落盘一律 best-effort：内存为准，保存失败不推翻已发出的 `done` 事件（错误返回给调用方记日志）；Load 遇坏行/超长行跳过该行继续，不让单行损坏锁死 session。
